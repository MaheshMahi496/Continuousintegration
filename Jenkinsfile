...
node {
  ...
  stage('SONAR Analysis') {
   ...
   stage('Function Test Cases execution') {
	   def v_path = "${env.WORKSPACE}/testScripts/CT-readyapi-project.xml"
	   echo "PATH IS : ${v_path}"
      // Run the maven build
      if (isUnix()) {
   ...
   if (currentBuild.currentResult == 'SUCCESS') {
        stage('Success') {
            ...
            emailext (
                ...
                to: "mahesh.rasakonda@whishworks.com,naresh.manthrabuddi@whishworks.com,aparna.deekonda@whishworks.com",
